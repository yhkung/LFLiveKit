...
- (NSString *)description {
    switch (self.status) {
        ...
        case STDPingStatusDidReceivePacket:
            //dhlu
            mTotalTimeMilliseconds+=self.timeMilliseconds;
            mNum++;
            //end dhlu.
...
+ (long)staticAvgRtridTime{
    
    if(0 == mNum)
        return 0;
    long cn = mTotalTimeMilliseconds/mNum;    
    return cn;
}



+ (CGFloat)getLossPercent{
    return mLPer;
}
...
+ (STDPingServices *)startPingAddress:(NSString *)address sendnum:(NSInteger)snum
    ...
    mLPer=0.0;
    //end dhlu
    STDPingServices *services = [[STDPingServices alloc] initWithAddress:address sendnum:snum];
